var LANGUAGE = {
	'l': {
		'titulo_principal': 'Generador de imágenes Ace Attorney',
		'titulo_navbar': 'Generador de imágenes Ace Attorney',
		'titulo_navbar_mobile': 'GIAA',
		'versao': 'v0.6a',
		'aba_botoes': 'Botones',
		'aba_botoes_menores': 'Botones menores',
		'aba_nome_prova': 'Nombre de la prueba / Perfil',
		'aba_subtitulo_prova': 'Subtítulo de la prueba / Perfil',
		'aba_descricao_prova': 'Descripción de la prueba / Perfil',
		'aba_sandbox': 'Sandbox',
		'rotulo_texto_botao': 'Texto del botón:',
		'rotulo_texto_botao_menor': 'Texto del botón menor:',
		'rotulo_texto_nome_prova': 'Texto del nombre de la prueba / perfil:',
		'rotulo_texto_subtitulo_prova': 'Texto del subtítulo de la prueba / perfil:',
		'rotulo_texto_descricao_prova': 'Texto de la descripción de la prueba / perfil:',
		
		'accordion_personalizacao_visual': 'Personalizacion Visual',
		'rotulo_lote': 'Por Lotes:',
		'rotulo_plataforma': 'Plataforma:',
		'rotulo_escala': 'Escala:',
		'rotulo_escala_automatica': 'Auto<span class="hidden-sm">matica</span>',
		'rotulo_fonte': 'Fuente:',
		'opcao_outra_fonte': 'Otra',
		'botao_negrito': 'N<span class="hidden-sm">egrita</span>',
		'botao_italico': 'I<span class="hidden-sm">tálico</span>',
		'rotulo_altura_linha': 'Altura de la línea:',
		'rotulo_claridade': 'Claridad:',
		'rotulo_margem_superior': 'Margen superior:',
		'rotulo_margem_esquerda': 'Margen izquierda:',
		'rotulo_mostrar_comparativo': 'Mostrar comparativo:',
		
		'opcao_fonte_automatica': 'Automatica',
		'opcao_fonte_normal': 'Normal',
		'opcao_fonte_condensada': 'Condensada',
		'opcao_fonte_extra_condensada': 'Extra Condensada',
		
		'botao_gerar': 'Generar',
		'botao_resetar': 'Resetear',
		
		'filtros': 'Filtros',
		'previa': 'Previsualización',
		'ultimos_canvas': 'Ultimos Canvas Creados',
		'sandbox_botoes_grandes': 'Botones grandes',
		'sandbox_botoes_menores': 'Botones menores',
		'sandbox_provas_perfis': 'Pruebas / perfis',
		
		'carregando': 'Cargando...'
	},
	'p': { // Placeholders
		'texto_botoes': 'Escribe para ver el resultado en la previsualización',
		'texto_lote': 'Escribe textos separados por quebras de linha',
		'texto_sandbox': 'Escribe para ver el resultado en la previsualización',
		'outra_fonte': 'Escribe aquí el nombre de la fuente'
	},
	't': { // Tooltips
		'about': 'Sobre el programa',
		'botao_negrito': 'Negrita',
		'botao_italico': 'Itálico',
	}
}